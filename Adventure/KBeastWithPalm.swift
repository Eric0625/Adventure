...
class KBeastWithPalm: KNPC{
    required init(k: KObject) {
        guard let beast = k as? KBeastWithPalm else {
            fatalError("Init KbeastWithPalm with unkown object")
        }
    ...
    override init(name: String){
        ...
        attackableLimbs = ["头部","颈部","胸口", "左掌", "右掌", "左前腿",
        ...
        defaultActions.append(KSkillAction(describe: "$A挥掌攻击$D的$l。\n", actionTypeOfDamage: DamageActionType.zhang))
        defaultActions.append(KSkillAction(describe: "$A往$D的$l一拍。\n", actionTypeOfDamage: DamageActionType.zhua))
        defaultActions.append(KSkillAction(describe: "$A往$D的$l狠狠地顶了一下。\n", actionTypeOfDamage: DamageActionType.yu))
